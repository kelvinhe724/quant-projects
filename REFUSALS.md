...
| **`rates-carry/` as a book sleeve** | **failed** | Through the book's own engine and cost model the sleeve is net Sharpe 0.53 with beta 0.75 to IEF+TLT and **alpha +1.4%/yr at t 1.66**, under the t>2 gate; its residual Sharpe of 0.33 is below the benchmark's own 0.39. It stays a standalone project and is not even counted as a book trial. | `rates-carry/README.md` §Through the engine, `framework/README.md` line 21, commit `890c72f` |
