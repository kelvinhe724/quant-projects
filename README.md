...
| project | what it does | headline result |
|---|---|---|
| `framework/` | The book: a daily target-weight simulator (as-of data guard, next-open fills, commission + spread + impact costs, vol targeting, drawdown and kill overlay) wrapped by skfolio, purgedcv, quantstats and alpaca-py; three ETF-only sleeves ported from the research (trend, FX carry, long/flat crypto trend), 1/N vs ERC allocation, purged walk-forward and deflated Sharpe with the real trial count, a signal-vs-beta attribution per sleeve, a v2 EWMAC candidate, a shadow paper daemon and an Alpaca paper bridge | Combined net Sharpe 0.26, 1.0%/yr at 4.3% vol, max drawdown -13.5%, PSR 0.89, DSR 0.06 over 22 trials; ERC does not beat 1/N out of sample so 1/N is live; the trend sleeve ports exactly (targets match the research project to 0 at all 266 month ends) but earns 0.43 vs 0.52 because next-open fills give up the month-end overnight gap; falsification: holding the ETF universe (Sharpe 0.59) beats the trend signal (0.41), and regressed on its own universe no v1 sleeve has significant alpha; the v2 EWMAC candidate scores 0.71 net (0.84 gross) with alpha t 3.2 against TrendETF's 0.41, on 19x turnover because its buffer is defeated, and stays out of the live book until an out-of-sample confirmation and the buffer fix |
