...
| `equity-xs/` | Cross-sectional equity model: LightGBM, ridge and their rank ensemble on fourteen point-in-time features (price, liquidity, three FRED macro series, membership) through `research/`'s feature store and walk-forward, scoring every S&P 500 member at month end on next-month return vs the index, long top decile / short bottom decile against plain 12-1 momentum, untouched 2022+ window | LightGBM net walk-forward OOS Sharpe 0.29 over 2011-2021 (rank IC 0.029, t 2.3), then -0.56 on the untouched 2022-2026 window (IC -0.008, t -0.4) while 12-1 momentum does the reverse (-0.15 then +0.23); a post-hoc ablation puts the walk-forward number on the three macro series, constant within a date, so the trees memorised regimes; the window was read three times and the README says what changed between reads |
...
| `options-collector/` | Daily after-close snapshot of the full option chain on SPY, QQQ, IWM and the ten largest SPY names, every expiry to 400 days, stored raw and never overwritten; own Black-76 inversion via the `svi/` cleaning pipeline, ATM and 25-delta skew per expiry, 30/60/90-day vols in total variance, basket implied correlation on the SPY row; launchd job written | First real snapshot 2026-09-04 (intraday, 36,406 quotes, 5.6 MB a day, 40/40 checks): SPY ATM 11.6 / 12.8 / 13.3% at 30 / 60 / 90 days against a VIX of 14.10, so the ATM-to-VIX basis the VRP backtest had to guess at is about 2.5 points; SPY 25-delta skew climbs from +1.0 vol points at 4 days to +5.8 at a year; implied correlation of the eight names with a clean slice -0.015, a level artifact of a ten-name sub-basket; JPM and BRK-B had no clean slice before the book filled in; nothing here is useful for a backtest until roughly 60 trading days for term structure and a year for a VRP or dispersion redo |
...
| `vol-models/` | Realised-vol horse race on SPY and 20 large names 2003-2026 through the lake and the research store: naive, HAR-RV, GARCH, GJR-GARCH and a pooled LightGBM at 5 and 21 days, rolling refits, Diebold-Mariano on QLIKE, then the best forecast used to time `vol-risk-premium/`'s short straddle through the research harness with an untouched last fifth | GJR wins on QLIKE (0.223 on the untouched window at 21 days, DM t -9.3 vs trailing vol, beats naive on 20 of 21 names) but not the VIX on SPY (t -1.2); timing the straddle on implied minus GJR makes the sleeve worse: untouched-window Sharpe 0.37 on capital vs 1.41 always-on, 15% time in market, DSR 0.52 vs 0.83 over 16 trials; the harness's active-day Sharpe picked a rule that never traded in three of four walk-forward test windows |
...
| `regimes/` | Regime overlay on `framework/`'s live beta+alpha book: Gaussian HMM on SPY (K in 2/3/4, causal forward filter, January refits from 2006) times a macro nowcast (curve slope, lagged payrolls, VIX), scaling each sleeve's 10% vol target, six variants selected by walk-forward against the bare book and opened once on the untouched last fifth | Not adopted: bare book net Sharpe 0.79 (DSR 0.87 over 65 trials) vs 0.53 (DSR 0.71) with the chosen K=3 overlay on the untouched window, drawdown -8.8% vs -11.7% but 22% more trades; walk-forward picked the overlay in 3 of 4 folds yet its stitched OOS Sharpe was 0.50 vs 0.59 for leaving the book alone |
...
| `nlp-events/` | FinBERT sentiment on 9,552 earnings 8-Ks (item 2.02) by point-in-time S&P 500 members, 2022-2026, against the reaction-day return and 1-20 session drift, purged walk-forward and an untouched last year through `research/`, 10 bps costs | Sentiment reads the release (reaction-day IC 0.094, t 4.6) but nothing survives to the first tradable open: IC within 0.01 of zero at every horizon, untouched-year net Sharpe -1.84 baseline / -0.77 sentiment / -1.09 residual, DSR 0.004 / 0.028 / 0.014 over 13 trials |
...
| `crypto-basis/` | Binance perp-spot basis on BTCUSDT and ETHUSDT, 5-minute bars 2024-01 to 2026-07 through the lake, long-spot/short-perp written as a total-return price with realised funding, z-score mean reversion through the research walk-forward and an untouched last fifth at taker, maker and VIP fees, plus a one-month OKX-vs-Binance minute monitor | Basis -4.5 bps with sub-bp noise since 2025; a 3-sigma dislocation is ~3 bps wide and gives back 1.2 bps a round trip gross against 34 bps of taker cost (9 at the top VIP tier); only 6 of 271,585 BTC bars ever exceed a taker round trip; gross Sharpe 4.4 pre-window, net -25.4 on the untouched window (-87.5%/yr taker, -40.9% VIP), DSR 0.00 over 8 trials; OKX-Binance gap never crossed a round trip in 44,160 minutes of July 2026 |
...
| `kalshi-model/` | Supervised calibration on the `prediction-markets/` snapshot plus 96 hours of candles per market (book, spread, path, volume, OI, clock, category -> P(YES)); logistic, boosted trees and a torch MLP times none/isotonic/Platt against the published bin fade, purged walk-forward through `research/`, untouched last fifth opened once, trades only where edge at the mid exceeds the whole quoted spread, touch plus taker fee | The mid wins: untouched window (2,318 markets, 2026-08-25 to 09-03) log-loss 0.5097 for the market vs 0.5136 best model and 0.5139 bin curve; walk-forward picked the bin curve over every estimator; every rule loses after the spread and fee, bin curve -6.5c/$ (t -2.95, 871 bets), model -5.6c (t -2.33, 684); models beat the mid only in books wider than 10c, where the rule never trades; DSR 0.001 over 20 trials on ten daily returns |
...
| `deep-lob/` | DeepLOB-style CNN+LSTM against an imbalance logistic and LightGBM, three classes with a dead band at 5/10/30 s, purged walk-forward and an untouched two-day window through `research/`, 1 bp taker fee, fill one second after the signal; the lake's L2 stream had 0.8 hours so the data is seven days of Binance spot aggTrades with the touch reconstructed from prints, no book | The network is the only model whose argmax leaves the flat class (macro F1 0.32 vs 0.29 for both baselines on the untouched days) and its gross edge is +0.13 bps a trade, inside the walk-forward's +0.15; at 1 bp a side every variant loses, the chosen one -17,180 bps over two days, DSR 0.00 over 14 trials; breakeven fee 0.09 to 0.13 bps a side |
...
| project | what it does | headline result |
|---|---|---|
| `framework/` | The book: a daily target-weight simulator (as-of data guard, next-open fills, commission + spread + impact costs, vol targeting, drawdown and kill overlay) wrapped by skfolio, purgedcv, quantstats and alpaca-py; three ETF-only sleeves ported from the research (trend, FX carry, long/flat crypto trend), an EWMAC rule and a vol-targeted 1/N core, four candidate books, 1/N vs ERC allocation, an engine-level position buffer, purged walk-forward, an untouched final window and deflated Sharpe with the real trial count, a signal-vs-beta attribution per sleeve, a shadow paper daemon and an Alpaca paper bridge | Live book since 2026-09-04 is beta+alpha (1/N of the 14 ETFs at 10% vol plus EWMAC, 1/2 each): net Sharpe 0.81 over 23 years, 5.7%/yr at 7.2% vol, max drawdown -12.1%, DSR 0.63 over 60 trials, 0.79 on the untouched last fifth of the panel against 0.40 for the v1 three-sleeve book; the win is mostly beta (the ETF universe held 1/N scores 0.98 on that window) and EWMAC alone fails its promotion test there (0.41 against TrendETF's 0.87, alpha t 0.75) after 0.68 with alpha t 3.1 on the full panel; moving the 10% buffer into the engine cut EWMAC's trade count 63% but its turnover only 4% (19.3x to 18.5x) and its net Sharpe 0.71 to 0.68, because the turnover is in the large daily moves of its own targets; ERC does not beat 1/N out of sample for any book; the trend sleeve ports exactly (targets match the research project to 0 at all 266 month ends) |
| `risk-model/` | Factor risk model for the desk: sample, Ledoit-Wolf, PCA (k from the Marchenko-Pastur edge with a Tracy-Widom cushion) and Fama-French observed-factor covariances raced on out-of-sample minimum-variance vol over the point-in-time S&P 500 (up to 480 names), its 100 most liquid and the book's 14 ETFs, 504-day windows, monthly, untouched last fifth opened once; the live book's exposures, contributions, 1-day 99% VaR/ES (parametric, historical, filtered historical with GARCH) with a Kupiec backtest of today's weights, and 2008-09 / 2020-03 / 2022 replays; `risk.exposures(positions, date)`, `risk.var(...)`, `risk.stress(...)` importable by `framework/book` | PCA wins the full panel (11.4% on the untouched window against 13.0% Ledoit-Wolf, 14.8% Fama-French, 33.6% sample at 31x leverage) and loses on the 14 ETFs, where plain sample wins (4.5% vs 6.1%); the live book runs 10.6% vol, 0.43 market beta, 55% of variance from the equity ETFs and 71% from Mkt-RF, 1-day 99% VaR 1.5% parametric / 1.6% historical / 1.9% FHS; today's weights lose 36.8% through Lehman to March 2009, 23.6% in the Covid month, 5.5% in 2022; parametric-normal VaR fails Kupiec (2.1% hit rate pre-window), FHS covers 1.2% pre-window and 1.0% on the window |

## Data & Research Infrastructure

| project | what it does | headline result |
|---|---|---|
| `data-lake/` | One Parquet store and one loader for every project: `lake.load(dataset, start, end, universe=, as_of=)`, with point-in-time S&P 500 filtering from `pit-universe/` and a hard as-of ceiling; launchd collectors for equities, FRED, Kalshi markets and books, EDGAR 8-Ks, crypto L2, klines and book depth, plus the options snapshots read through `options-collector/` | First run 2026-09-05: 9 datasets, 183 MB (150 of it 3,355,162 equity rows across 680 tickers, 2003-2026); FRED 192,858 rows over 20 series back to 1939; crypto book depth 2,004,480 rows |
| `research/` | Point-in-time feature store with a peek audit (`build(audit=True)` perturbs one session and requires every earlier feature row byte-identical), experiment registry that feeds the deflated Sharpe with the real trial count, `Alpha` interface that plugs into `framework/`'s engine, model registry, and an untouched final window that opens once | Worked example: 12-1 momentum on the book's 14 ETFs scores research-path Sharpe 0.23 over 2004-2021 (DSR 0.59 over 5 trials), 0.44 on the untouched 2022-2026 window, but walk-forward selected-in-sample Sharpe 0.04; 46/46 offline checks; the audit catches a `shift(-1)` and a centred window |
