...
Run 2026-09-04 02:06, panel hash `aa436bbc6c2e4712`, 2004-07-01 to 2026-08-31 (22.1 years), shadow cost model, kill switch off for backtests (see below).

## Headline (book, live allocator, net)

| Sharpe | ann. return | ann. vol | max drawdown | PSR | DSR | trials | MinBTL | MinTRL |
|---|---|---|---|---|---|---|---|---|
| 0.262 | 1.04% | 4.30% | -13.5% | 0.890 | 0.207 | 16 | 47.2 y (have 22.1) | 39.8 y |

DSR uses n_trials = 16: every configuration this file runs on the real panel (sleeves, the trend grid, allocators, cost and delay variants, kill on) is appended to `trials.csv` (45 rows so far) and identical return streams count once; the variance of their daily Sharpes is 2.34e-04. The per-asset vol target is undone by the sleeve-level 10% target, so the trend grid is really four lookbacks. The 20 placebo draws are a null, not candidates; counting them too gives DSR 0.113. PSR is the probability the true Sharpe is above zero; DSR the same after deflating for selection. MinTRL is the track length needed to reject zero at 95%.
...
Tearsheet: `reports/tearsheet.html` (quantstats, SPY benchmark). 269s.
