...
Shadow cost model throughout: 5 bps commission, 2 bps half spread, sqrt impact (k = 0.1), 50 bps a year borrow on shorts, next-open fills. Overlay per sleeve: 10% vol target, 3x gross cap, half size past a 15% drawdown, kill at 25% (off for backtests), and since v3 a **10% position buffer on the final weights**: after the overlay has scaled a sleeve's targets, an instrument trades only when its held weight is outside target x (1 +/- 0.1); a gross or per-instrument breach, a change in the drawdown cut or a kill sends every target regardless. The Alpaca bridge applies the same band against the paper account's positions. **One order model, both paths** (2026-09-09): `engine/execution.py::OrderRules.target_quantities` is the only implementation of what a broker does to a target weight, and `book_config` hands the same object to the engine's `Executor` and to `book/broker.py::build_orders`, so a backtest position is a position the paper account could also reach: the 10% buffer, a $25 minimum order, whole shares on a short leg (Alpaca will not take a fractional short), and a short target on crypto flattening to zero rather than going short. Before this the three latter rules existed only in `build_orders` and no published number had ever seen them; the "was" column in the Headline is the size of that gap. `book/check.py` asserts the two paths land on the same book for a hand-made set of weights and for 300 random ones. Sharpe is on daily returns, days before a sleeve is live excluded. **Gross** is the same path with each day's trade costs and borrow added back; a separate zero-cost run is no longer the same path, because the buffer trades off held weights and those depend on what costs have done to equity (a zero-cost TrendETF run lands at 0.41 against 0.46 net for that reason, which is noise, not a cost; both figures predate 2026-09-09).

## Headline

Sleeves alone, kill off, buffered engine, **shared order model** (2026-09-09, below), whole panel. The "was" column is the same panel through the old execution path (`OrderRules()`, every broker rule off), which is the path every number in this file used before 2026-09-09; it reproduces the previous headline to a rounding place.

| | Sharpe gross | Sharpe net | was (net) | ann. return net | ann. vol | max drawdown | live from | trades |
|---|---|---|---|---|---|---|---|---|
| TrendETF | 0.48 | 0.40 | 0.46 | +3.0% | 8.3% | -20.7% | 2004-07 | 2,273 |
| FXCarryETF | -0.08 | -0.16 | -0.17 | -1.2% | 6.1% | -38.5% | 2006-07 | 422 |
| CryptoTrend | 0.10 | 0.09 | 0.09 | +0.4% | 9.4% | -16.3% | 2022-02 | 50 |
| EWMAC (from 2005-06) | 0.87 | 0.72 | 0.68 | +7.1% | 10.2% | -19.3% | 2005-06 | 25,929 |
| ETFBeta | 0.68 | 0.67 | 0.66 | +5.4% | 8.4% | -26.7% | 2003-06 | 3,633 |
| v1 book, 1/3 each, static | 0.37 | 0.26 | 0.25 | +1.0% | 4.3% | -12.2% | 2004-07 | 2,741 |
| **beta+alpha book, 1/2 each, static (live)** | 0.92 | **0.81** | 0.81 | +5.7% | 7.2% | -12.1% | 2003-06 | 29,342 |

**The kill switch is off in every number above and on in the daemon.** `book_config(kill=True)` is the live default; every backtest published here runs `kill=False`, so the reported system is not the deployed one. The size of that gap, same panel, same shared order model: the live book is **0.70 net with the kill on against 0.81 with it off** (+4.6% a year, 6.8% vol, -13.6% max drawdown, 27,687 trades); the v1 book is 0.29 with it on against 0.26 with it off. A 25% sleeve drawdown flattens that sleeve for the rest of the backtest, which is why the live book's drawdown is deeper with the kill on than without it. Live, a kill stops the sleeve until a human restarts it, which no backtest models; the honest reading of the headline is that the deployed configuration's backtest Sharpe is 0.70, and 0.81 is the same book with a safety feature disabled.

Book honesty (`book/reports/validation.md`, run 2026-09-05 02:59, i.e. **before** the order model was shared; every table below this line and outside the Headline is from that run and will move by roughly the amounts in the "was" column when `validate.py` is next run in full): live book PSR 1.00, **DSR 0.63** with n_trials = 60 (every configuration `validate.py` has run on the real panel, before and after the buffer moved into the engine, identical return streams counted once; 0.57 if the 20 placebo draws are counted too), MinBTL 8.4 years against 23.2 in hand, MinTRL 4.3 years. Those last three are what a 0.8 Sharpe over 23 years buys, and for a book whose core is 0.67 beta to its universe they are statements about the universe. The v1 book on the same engine: 0.25 net, PSR 0.88, DSR 0.02 at 47 trials (the count when it was the live book, from the run before this one). Cost stress on the live book: 2x costs 0.73, 4x 0.55. A one-bar signal delay does nothing (0.82; +5 bars 0.76), which is what a book that is mostly a buy-and-hold core looks like.
...
| Rates | FRED (OECD `IR3TIB01*`) | Monthly averages published with a lag that differs by currency, so `universe.load_rates` truncates every column to the last month all seven currencies have: one carry rank never mixes vintages. As of 2026-09-09 FRED's OECD prints end 2026-01 for EUR and GBP, 2026-05 for JPY and 2026-06 for the rest, so every leg ranks on the January 2026 average — a 7-month uniform lag, and the honest cost of the fix. The EUR/GBP-only ECBDFR/SONIA splice was removed on 2026-09-09 — it ranked live EUR and GBP against three-month-old everything else. The upgrade is a per-currency overnight series for all of them (ESTR, SONIA, TONAR, RBA cash, CORRA, SARON, EFFR), not two of seven. `book/check.py` asserts the columns share an end month. The daemon re-downloads every series each run. |
